...
export default function PinDetailView({ pin: initialPin }: PinDetailViewProps) {
  ...
  /* ── Unified masonry layout: hero + related pins as one continuous board ── */
  const unifiedRef = useRef<HTMLDivElement>(null)
  const heroElRef = useRef<HTMLDivElement>(null)
  const pinElRefs = useRef<(HTMLDivElement | null)[]>([])
  const [containerW, setContainerW] = useState(0)
  const [pinPositions, setPinPositions] = useState<{ top: number; left: number; width: number }[]>([])
  const [layoutH, setLayoutH] = useState(0)
  const [heroLayoutW, setHeroLayoutW] = useState(0)

  const COL_TARGET = 236
  const GRID_GAP = 14

  // Track container width
  useEffect(() => {
    const el = unifiedRef.current
    if (!el) return
  ...
  const itemCount = loadingSimilar ? 12 : similarPins.length
  ...
  const calcLayout = useCallback(() => {
    ...
    const totalCols = Math.max(1, Math.floor((containerW + GRID_GAP) / (COL_TARGET + GRID_GAP)))
    const colW = totalCols > 1
      ? (containerW - GRID_GAP * (totalCols - 1)) / totalCols
      : containerW

    // Hero spans ~40-50% of columns on desktop (min 2), full width on mobile
    const heroSpan = totalCols >= 3
      ? Math.min(totalCols - 1, Math.max(2, Math.round(totalCols * 0.45)))
      : totalCols
    const hW = heroSpan * colW + Math.max(0, heroSpan - 1) * GRID_GAP

    // Set hero width directly and force synchronous reflow to measure height
    heroEl.style.width = `${hW}px`
    const heroH = heroEl.offsetHeight

    // Initialize column heights — hero columns start offset by hero height
    const colHeights = new Array(totalCols).fill(0) as number[]
    for (let c = 0; c < heroSpan; c++) {
      colHeights[c] = heroH + GRID_GAP
    }

    const newPos: { top: number; left: number; width: number }[] = []
    for (let i = 0; i < itemCount; i++) {
      const el = pinElRefs.current[i]
      const itemH = el?.offsetHeight ?? 280
      ...
      newPos.push({
        ...
        left: shortest * (colW + GRID_GAP),
        width: colW,
      })
      colHeights[shortest] += itemH + GRID_GAP
    }

    const maxH = Math.max(...colHeights, 0)
    setHeroLayoutW(hW)
    setPinPositions(newPos)
    setLayoutH(maxH > GRID_GAP ? maxH - GRID_GAP : 0)
  }, [containerW, itemCount])

  useEffect(() => {
    const frame = requestAnimationFrame(calcLayout)
  ...
  useEffect(() => {
    const el = unifiedRef.current
    if (!el) return
    const handler = () => requestAnimationFrame(calcLayout)
    el.addEventListener('load', handler, true)
    return () => el.removeEventListener('load', handler, true)
  }, [calcLayout])

  const hasPositions = pinPositions.length === itemCount && itemCount > 0
    ...
    <>
      <style>{`
        .pdv-page {
          max-width: 1800px;
          margin: 0 auto;
          padding: 12px clamp(16px, 2vw, 48px) 60px;
        }
        .pdv-hero-card {
          border-radius: 16px;
          background: var(--bg);
        ...
        .pdv-hero-media img, .pdv-hero-media video {
          width: 100%;
          height: auto;
          display: block;
          object-fit: contain;
      ...
      <div className="pdv-page">
        {/* ═══ UNIFIED BOARD: hero + related pins as ONE continuous masonry ═══ */}
        <div
          ref={unifiedRef}
          style={{
            position: 'relative',
            width: '100%',
            height: layoutH || undefined,
            minHeight: 400,
          }}
        >
          {/* ── HERO PIN (top-left, spans multiple columns) ── */}
          <div
            ref={heroElRef}
            className="pdv-hero-card"
            style={{
              position: 'absolute',
              top: 0,
              left: 0,
              width: heroLayoutW || '100%',
            }}
          >
            {/* Action bar */}
            <div className="pdv-action-bar">
              <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <LikeButton pinId={pin.id} currentUserId={userId ?? undefined} onAuthRequired={() => { window.location.href = '/login' }} iconSize={20} />
                <button onClick={() => commentRef.current?.scrollToComments()} title="Comments" style={iconBtnStyle}
                  onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--text)'; e.currentTarget.style.background = 'var(--brume)' }}
                  onMouseLeave={(e) => { e.currentTarget.style.color = 'var(--muted)'; e.currentTarget.style.background = 'transparent' }}
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 21a9 9 0 1 0-9-9c0 1.488.36 2.89 1 4.127L3 21l4.873-1c1.236.64 2.64 1 4.127 1z" /></svg>
                </button>
                ...
                {pin.linkedin_url && (
                  <a href={pin.linkedin_url} target="_blank" rel="noopener noreferrer" title="LinkedIn" style={{ ...iconBtnStyle, textDecoration: 'none' }}
                  ...
                  ><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg></a>
                )}
                {pin.tiktok_url && (
                  <a href={pin.tiktok_url} target="_blank" rel="noopener noreferrer" title="TikTok" style={{ ...iconBtnStyle, textDecoration: 'none' }}
                    onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = 'var(--text)'; (e.currentTarget as HTMLElement).style.background = 'var(--brume)' }}
                    onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = 'var(--muted)'; (e.currentTarget as HTMLElement).style.background = 'transparent' }}
                ...
                {pin.instagram_url && (
                  <a href={pin.instagram_url} target="_blank" rel="noopener noreferrer" title="Instagram" style={{ ...iconBtnStyle, textDecoration: 'none' }}
                    onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = 'var(--text)'; (e.currentTarget as HTMLElement).style.background = 'var(--brume)' }}
                    onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = 'var(--muted)'; (e.currentTarget as HTMLElement).style.background = 'transparent' }}
                  ><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg></a>
                )}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {isOwner && (
                  <button type="button" onClick={handleEditClick} title="Edit pin" style={{ ...iconBtnStyle }}
                    onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--text)'; e.currentTarget.style.background = 'var(--brume)' }}
                    onMouseLeave={(e) => { e.currentTarget.style.color = 'var(--muted)'; e.currentTarget.style.background = 'transparent' }}
                  ><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
                )}
                {userId !== undefined && (
                  <button onClick={handleSave} disabled={saving || !userId}
                    title={!userId ? 'Sign in to save' : saved ? 'Unsave' : 'Save'}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 18px', borderRadius: 20, border: 'none', background: saved ? 'var(--menthe)' : 'var(--verveine)', color: '#fff', fontSize: '0.875rem', fontWeight: 700, cursor: saving || !userId ? 'default' : 'pointer', opacity: !userId ? 0.5 : 1, transition: 'background 150ms, opacity 150ms' }}
                  ...
                  >
                    {saving ? (
                      ...
                      <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" /></svg>Save</>
                    )}
                  </button>
                )}
              </div>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ perspective: '1200px' }}>
                <div style={{ position: 'relative', transformStyle: 'preserve-3d', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)', transition: 'transform 0.45s ease', cursor: 'pointer' }}>
                  <div className="pdv-hero-media" onClick={() => setFlipped(true)} style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', position: 'relative' }}>
                    ...
                    <div style={{ position: 'absolute', bottom: 12, left: 14, fontSize: '0.75rem', color: 'rgba(255,255,255,0.9)', fontWeight: 600, letterSpacing: '0.04em', background: 'rgba(0,0,0,0.35)', borderRadius: 8, padding: '4px 10px', backdropFilter: 'blur(4px)' }}>click to flip</div>
                  </div>
                  <div onClick={() => setFlipped(false)} style={{ position: 'absolute', inset: 0, transform: 'rotateY(180deg)', backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', background: '#fff', padding: '24px 22px', display: 'flex', flexDirection: 'column', gap: 12, overflowY: 'auto', pointerEvents: flipped ? 'auto' : 'none' }}>
                    <div style={{ textAlign: 'left', fontSize: '0.75rem', color: 'var(--muted-light)', fontWeight: 600 }}>↩ click to flip back</div>
                    {categories.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {categories.map((cat) => (<span key={cat.id} style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.08em', color: 'var(--menthe)' }}>{cat.label}</span>))}
                      </div>
                    )}
                    <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text)', lineHeight: 1.3, margin: 0 }}>{pin.title || 'Untitled'}</h2>
                    {pin.description && (<p style={{ fontSize: '0.9375rem', color: 'var(--muted)', lineHeight: 1.65, margin: 0, whiteSpace: 'pre-wrap' }}>{pin.description}</p>)}
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 4 }}>
                      <a href={pin.live_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{ flex: 1, minWidth: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px 14px', borderRadius: 14, background: 'var(--menthe)', color: '#fff', fontSize: '0.875rem', fontWeight: 700, textDecoration: 'none', transition: 'opacity 150ms' }}
                        onMouseEnter={(e) => { e.currentTarget.style.opacity = '0.85' }} onMouseLeave={(e) => { e.currentTarget.style.opacity = '1' }}
                      ><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>Visit Live</a>
                      {pin.repo_url && (
                        <a href={pin.repo_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px 14px', borderRadius: 14, border: '1.5px solid var(--border)', background: 'transparent', color: 'var(--text)', fontSize: '0.875rem', fontWeight: 600, textDecoration: 'none', transition: 'border-color 150ms, background 150ms' }}
                          onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--menthe)'; e.currentTarget.style.background = 'var(--brume)' }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'transparent' }}
                        ><svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.477 2 2 6.477 2 12c0 4.418 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.009-.868-.013-1.703-2.782.604-3.369-1.341-3.369-1.341-.454-1.155-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0 1 12 6.836a9.59 9.59 0 0 1 2.504.337c1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.163 22 16.418 22 12c0-5.523-4.477-10-10-10z" /></svg>Repo</a>
                      )}
                    </div>
                  </div>
                </div>
            ...
            {/* Metadata: author, title, comments */}
            <div style={{ padding: '16px 18px 20px' }}>
              {pin.profile && (
                <Link href={`/profile/${pin.profile.username}`} style={{ textDecoration: 'none', display: 'block', marginBottom: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 8px', borderRadius: 12, transition: 'background 150ms', cursor: 'pointer' }}
                    onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--brume)' }} onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent' }}
                  >
                    ...
                    <div style={{ minWidth: 0 }}>
                      <p style={{ margin: 0, fontSize: '0.875rem', fontWeight: 700, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{pin.profile.display_name || pin.profile.username}</p>
                      <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--muted)' }}>@{pin.profile.username}</p>
                    </div>
                  </div>
                </Link>
              )}
              <h1 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text)', lineHeight: 1.25, margin: '0 0 6px', letterSpacing: '-0.02em' }}>{pin.title}</h1>
              {pin.description && (<p style={{ fontSize: '0.8125rem', color: 'var(--muted)', lineHeight: 1.6, margin: '0 0 14px', whiteSpace: 'pre-wrap' }}>{pin.description}</p>)}
              <div style={{ height: 1, background: 'var(--text)', opacity: 0.06, margin: '0 0 14px' }} />
              <CommentSection ref={commentRef} pinId={pin.id} pinOwnerId={pin.owner_id} />
            </div>
          </div>

          {/* ── RELATED PINS (positioned by unified masonry algorithm) ── */}
          {Array.from({ length: itemCount }).map((_, i) => {
            const pos = hasPositions ? pinPositions[i] : null
            return (
              <div
                key={loadingSimilar ? `skel-${i}` : similarPins[i]?.id ?? i}
                ref={(el) => { pinElRefs.current[i] = el }}
                style={pos
                  ? { position: 'absolute', top: pos.top, left: pos.left, width: pos.width }
                  : { position: 'absolute', top: 0, left: 0, width: COL_TARGET, opacity: 0, pointerEvents: 'none' as const }
                }
              >
                {loadingSimilar ? (
                  <div className="animate-pulse" style={{ height: 140 + (i % 3) * 60, borderRadius: 16, background: 'var(--menthe-light)', opacity: 0.4 }} />
                ) : similarPins[i] ? (
                  <PinCard pin={similarPins[i]} currentUserId={userId ?? undefined} initialLikeCount={likesMap[similarPins[i].id]?.likeCount} initialLikedByMe={likesMap[similarPins[i].id]?.likedByMe} />
                ) : null}
              </div>
            )
          })}
