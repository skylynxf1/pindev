...
export default function PinModal({ pinId }: PinModalProps) {
  ...
  // undefined = auth not yet checked, null = logged out, string = userId
  ...
  return (
        ...
        {!loading && pin && (
          <>
            ...
            <div className="flex flex-col flex-1 overflow-y-auto p-7 gap-5 max-h-[80vh]">
              ...
              {/* Title */}
              {pin.title && (
                <h1 className="text-xl font-bold text-[#0F1720] leading-snug pr-10">
                  {pin.title}
                </h1>
              )}
              ...
              {pin.profile && (
                <Link
                  ...
                  onClick={handleClose}
                  className="flex items-center gap-3 rounded-2xl border border-[#E6ECEA] p-3 hover:bg-[#EDF7BE]/50 transition-colors"
                >
                  ...
                  <svg className="ml-auto flex-shrink-0 text-[#5B6B73]" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                {userId !== undefined && (
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-semibold text-white transition-colors ${
                      saved
                        ? 'bg-[#35C8B4] cursor-default'
                        : !userId
                          ? 'bg-[#A4CF4A]/50 cursor-not-allowed'
                          : 'bg-[#A4CF4A] hover:bg-[#35C8B4]'
                    }`}
                  >
                    {saved ? (
                      <>
    ...
    {/* Board picker — portal renders above the modal */}
    {showPicker && pin && (
      <BoardPickerModal
        pin={pin}
